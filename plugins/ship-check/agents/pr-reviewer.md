...
2. **Load code standards + preference recall** — use ToolSearch to load the vault-cortex
   ...
   Then read all four cross-project standards notes (distilled current consensus):
   ...
   Then recall the dated evidence trail for the PR's domain — it surfaces preferences
   newer than the notes: `vault_memory_recall({ query: "<change domain, e.g. 'error handling'>" })`

3. **Load sequential thinking** for organizing findings:
   `ToolSearch({ query: "select:mcp__sequential-thinking__sequentialthinking" })`

4. **Read the full diff** and all source files touched by the PR.
